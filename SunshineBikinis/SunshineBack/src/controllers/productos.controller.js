...
export const createProducto = async (req, res) => { 
  ...
  try {   
    const [rows] = await pool.query('INSERT INTO productos (codigo_Producto, nombre_Producto, descripcion_Producto, categoria_Producto, precioCompra_Producto,precioVenta_Producto, StockProducto, thumbnail) VALUES (?,?,?,?,?,?,?,?)', [codigo_Producto, descripcion_Producto, categoria_Producto, nombre_Producto, parseFloat(precioCompra_Producto), parseFloat(precioVenta_Producto), StockProducto, req.file.filename])
...
export const deleteProducto = async (req, res) => {
  try {
    ...
    if (rows.affectedRows > 0) {
      res.status(204).json({ message: "Producto eliminiado" })
    }
